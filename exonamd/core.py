...
def compute_namd(userid: str, df: pd.DataFrame, Npt: int, do_plot: bool = True, seed: int = 42, return_samples: bool = False):
    ...
    if do_plot:
        ...
        plt.vlines(log_quantiles, 0, plt.ylim()[1], color=["red", "black", "red"], linestyles="dashed")
        plt.xlabel(rf"log(NAMD)")
        plt.ylabel("Relative frequency")
        title = f"{userid}: " + rf"log(NAMD) = ${log_quantiles[0.5]:.2f}^{{+{log_quantiles[0.84]-log_quantiles[0.5]:.2f}}}_{{-{log_quantiles[0.5]-log_quantiles[0.16]:.2f}}}$"
        plt.title(title)
        plt.show()
